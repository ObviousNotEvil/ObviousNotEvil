...
date last modified by author: 16/1-2024  */ // only utf-8 support
...
function BsTB(txt){let new_str=StB(txt).match(/.{1,8}/g),result='',str;
	for(str of new_str){result+=str[4]+str[5]+str[6]+str[7]+str[0]+str[1]+str[2]+str[3];
	};return result
};
async function hr(str,phrase){let b0=0,b1=Math.abs(str.length+phrase.length),b2=a.i,b3=0;
	...
	if(Number.isInteger(str.length/3)){a1=a1.split('').reverse().join('')}
...
async function reorder(str,phrase){
	...
	for(a2;a2<a0.length;a2++){a3=a1[a2];a1.push(a3);a4=a0[a2].split('');
